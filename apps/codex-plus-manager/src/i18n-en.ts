...
export const EN_PLAIN: Record<string, string> = {
  ...
  "Responses reasoning 兼容": "Responses reasoning compatibility",
  "原样透传": "Pass through",
  "OpenAI opaque 兼容": "OpenAI opaque compatibility",
  "原生任务兼容": "Native task compatibility",
  "Responses 默认启用": "Enabled by default (Responses)",
  "「Responses 默认启用」表示 Responses 协议下默认开启，不做远端能力探测；转发时用明文协议保留原生子任务消息与工具参数。": "\"Enabled by default (Responses)\" means the feature is on by default for the Responses protocol without remote capability probing; requests are forwarded with native subagent messages and tool arguments preserved in plaintext.",
  "始终启用": "Always enabled",
  "请求转发到该供应商时，用明文协议保留原生子任务消息与工具参数。": "When requests are forwarded to this provider, preserve native subagent messages and tool arguments in plaintext.",
  "Responses 结构透传": "Responses wire policy",
  "兼容改写": "Compatible rewrites",
  "结构透传": "Passthrough",
  "结构透传：跳过 namespace/ID/原生子任务等兼容改写，reasoning 兼容选项由该策略接管，按原样透传。": "Passthrough: skips namespace/ID/native-subagent compatibility rewrites; the reasoning compatibility option is governed by this policy and reasoning items are passed through as-is.",
  "兼容改写：保持既有兼容能力（namespace 扁平化、ID 规范化、原生子任务别名）。": "Compatible rewrites: keeps the existing compatibility capabilities (namespace flattening, ID normalization, native subagent aliasing).",
  "Custom 工具转 Function": "Wrap custom tools as functions",
  "适用于不完整支持 custom 工具的上游。将 custom 工具包装为包含 input 字符串的 function，返回时还原为 custom 调用；不会更换供应商或模型。": "For upstreams with incomplete custom-tool support. Custom tools are wrapped as functions with an input string and restored back to custom calls in responses; the provider and model stay unchanged.",
  "配置冲突：Custom 工具转 Function 与结构透传互斥，保存后请求会被拒绝。请关闭其中一项。": "Conflict: wrapping custom tools as functions is mutually exclusive with passthrough; requests will be rejected. Turn one of them off.",
  "启用后工具输入字符串可往返保留，但原 custom 语法约束可能退化为文字指导和本地校验；流式工具参数将在完整校验后交付，可能略晚显示。": "Tool input strings round-trip exactly, but original custom grammar constraints degrade to textual guidance plus local validation; streamed tool arguments are delivered after full validation, so they may appear slightly later.",
  "「Custom 工具转 Function」与「结构透传」互斥：请关闭其中一项。": "\"Wrap custom tools as functions\" and \"Passthrough\" are mutually exclusive: turn one of them off.",
  "移除 reasoning": "Strip reasoning",
  "仅 Responses API 生效": "Only effective for the Responses API",
  "仅 Responses API 生效。": "Only effective for the Responses API.",
  "仅处理 Responses 顶层 input 的 reasoning 条目；Chat Completions 不受影响。":
    "Only normalizes top-level reasoning items in Responses input; Chat Completions is unaffected.",
  "上一页": "Previous page",
  "下一页": "Next page",
  ...
  "失败切换": "Failover",
  "传输失败": "Transport failure",
  "冷却状态": "Cooldown status",
  "重置冷却": "Reset cooldowns",
  "重置": "Reset",
  "重置中": "Resetting",
  "未冷却": "Not cooling down",
  "当前使用": "In use",
  "冷却后下一顺位": "Next after cooldown",
  "当前聚合未启用": "This aggregate is not active",
  "暂无冷却记录": "No cooldown history",
...
export const EN_TEMPLATE: Record<string, string> = {
  ...
  "{0} 次": "{0} times",
  "冷却 {0}": "Cooldown {0}",
  "连续失败 {0}/{1}": "Consecutive failures {0}/{1}",
  "上次原因：{0}": "Last reason: {0}",
