...
服务器为 Python 标准库单文件，无需 pip；通过子进程调用同一原生 CLI。将以下条目加入另一个 AI 客户端的 MCP 配置，替换实际绝对路径：
...
```json
{
  "mcpServers": {
    "codexpp-control": {
      "command": "python",
      "args": ["C:/实际路径/codexpp-control/scripts/control.py", "--manager", "C:/实际交付目录/codex-plus-plus-manager.exe", "--mcp"]
    }
  }
}
```

暴露 14 个 `codexpp_*` 工具，名字由上表命令的连字符替换为下划线。工具输入与 CLI 一致；写操作附加 `dryRun: true` 可预览。MCP stdout 仅输出 JSON-RPC，调用失败通过 isError 返回。
